...
exports.updateUserVocab = async (req, res) => {
  const { userVocabId, isCorrect } = req.body;

  console.log("updateUserVocab", userVocabId, isCorrect);


  if (!userVocabId || typeof isCorrect !== "boolean") {
    return apiResponse(res, {
      code: responseCode.INVALID_INPUT.code,
  ...
  try {
    const userVocab = await require("../models").UserVocab.findByPk(
      userVocabId
    );
    if (!userVocab)
      return apiResponse(res, {
        code: responseCode.NOT_FOUND.code,
        mess: "Không tìm thấy từ vựng của người dùng",
      });
